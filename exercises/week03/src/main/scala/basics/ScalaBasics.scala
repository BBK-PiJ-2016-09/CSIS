...
object ScalaBasics {
  ...
  def minRecursive(r: Array[Int]): Int = {
    def minRecHelper(r: Array[Int], element: Int, minimum: Int): Int = {
      if(r.length > 2) {
        return minRecHelper(r.tail, r.tail.head, math.min(minimum, r.head))
      }
      if (r.length ==2) {
        return math.min(minimum, r.tail.head)
      }
      else {
        return math.min(minimum, r.tail(0))
      }
    }
    if (r.length > 2){
      return minRecHelper(r.tail.tail, r.tail.head, r.head)
    }
    if (r.length == 2) {
      return math.min(r.head, r.tail.head)
    }
    else {
      return r.head
    }
  ...
  def isPalindrome(s: String): Boolean = s match  {
    ...
    case s if ",.-!?; ".contains(s.head) => isPalindrome(s.tail)
    case s if (",.-!?; ".contains(s(s.length - 1))) => isPalindrome(s.substring(0, s.length - 1))
    case s if s.head == s(s.length - 1) => isPalindrome(s.substring(1, s.length - 1))
    case _ => false
  }
